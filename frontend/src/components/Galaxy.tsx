...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // Force gold/orange color for all stars - hue 40 (orange-gold), high saturation
      float goldHue = (uHueShift / 360.0);
      float goldSat = uSaturation;
      float goldVal = min(val * 1.2, 1.0); // Slightly brighter for gold effect
      base = hsv2rgb(vec3(goldHue, goldSat, goldVal));
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      if (!disableAnimation) {
        ...
        program.uniforms.uStarSpeed.value = (elapsed * starSpeed) / 10.0;
      } else {
        startTimeRef.current = null;
      }
    ...
    return () => {
      ...
      // Lose context only after canvas is removed
      try {
